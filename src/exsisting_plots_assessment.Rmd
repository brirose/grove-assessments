...
```
# Overview {.tabset}
Grove assessments are planned for each Sequoia grove within the boundary of Sequoia Kings Canyon NP. These assessments will allow for the prioritization of treatment efforts within the park to ensure the protection of this valued resource.

The assessments will be informed by data from SEKI's longstanding fuels monitoring program, bolstered by data from other projects concerned with fuels in the park. These include projects done by SEKI's fire effects program, USGS, and the Shive Lab at UCB in partnership with SEKI. In addition utilizing existing plots, additional data will be collected at plots added as needed throughout the groves.

This overview outlines the existing plots in the park that can be used for grove assessment.
...
There are `r nrow(groves_seki_buffered)` groves within the SEKI boundary ranging from `r round(min(groves_seki_summary$groveacres))` acres to `r round(max(groves_seki_summary$groveacres))` acres with a total of `r round(max(groves_seki_summary_total$groveacres))` acres. The smallest groves are defined by individual trees (Putnam-Francis Tree and Fenced Tree while the largest are thousands of acres (Redwood Mountain and Giant Forest). The median grove size is `r round(median_grove)` acres and the mean is `r round(median_grove)`.
...
groves_summary <- all_plots_groves %>% 
  ...
  mutate(plots_peracre_buf = round(Total/grove_wbuffer_acres,2),
         acres_perplot_buf = round(grove_wbuffer_acres/Total, 2))
```
...
ggplot(fire_plot_recent, aes(burnsev))+
...
1. What factors are important to represent/stratify? *Our initial thought is that aspect will be the most important variable and that it is worth considering topographic position*
...
3. How should we prioritize groves and the placement of plots therein?
