...
for(i in 1:length(shapes.list)) {
  ...
  write.csv(draw_grts_base_and_over[,c(1,2,4,5,10:13,15:18)], here(paste("outputs/grts/grts_base_and_over_",gsub('.{4}$', '', as.character(shape.is)))))
...
grts.groves.table = grts.groves %>%
  ...
  group_by(strt_nm,grove_name,siteuse) %>%
  summarise(num.grts.plots = length(strt_nm), 
            NEW.plots.needed = mean(NEW_pl_)) %>%
  st_drop_geometry() %>%
...
# #write out csv
# write.csv(grts.groves,(here("data/spatial_data/outputs/grts_points_easier_groves_table_summary.csv")))
...
grid200 <- needed.strata.plots_easier %>% 
  st_make_grid(cellsize = c(200,200), what = "centers") %>% # grid of points
  st_as_sf() %>%
  st_intersection(needed.strata.plots_easier) %>%
  mutate(study.design = "grid")
head(grid200)

# write_sf(grid200,(here("data/spatial_data/outputs/grid200m_points_easier_groves_4May2025_wstrata.shp")),
#          driver="ESRI Shapefile", overwrite_layer=TRUE)
